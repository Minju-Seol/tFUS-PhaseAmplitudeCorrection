...
skull = data['skull'].to(device) #[12, 264, 264, 164]
td_vxl = data['td_vxl'].to(device) #[12, 1024, 3]
so_vxl = data['so_vxl'].to(device)
ph_list =  data['ph_list'].to(device)
amp_list =  data['amp_list'].to(device)
...
with torch.no_grad():
    for target_point_idx in test_points:
        ...
        print('---------------------------')
        print(f'MAE: {amp_mae/1000:.6f} kPa')
