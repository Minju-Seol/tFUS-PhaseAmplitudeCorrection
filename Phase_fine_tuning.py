...
for td_idx in range(1024):
    ...
    ref_dataset = MultiSkullDataset(
        ...
        ph_list=ph_list, target_class=target_class, td_idx_list=[td_idx], so_idx_list=fine_tune_points
    )
    ref_loader = DataLoader(ref_dataset, batch_size=16, shuffle=True)
    for m in model.phase_head.mlp.modules():
    ...
    for fine_epoch in range(fine_tune_epochs):
        ...
        for skull_idx, so, td, gt_phase, soft_label, vol in ref_loader:
            so, td, gt_phase, soft_label = so.to(device), td.to(device), gt_phase.to(device), soft_label.to(device)
            vol = vol.to(device)
            td = torch.round(td)

            _, entry, exit = extract_u_centered_cube(vol, td, so, size_hwd=(20,20,128), step_hwd=(1.0,1.0,1.0), td_anchor=(10,10,2), align_corners=True)
            hu_data = sample_hu_line(vol, entry, exit, td, so, N=128, align_corners=True)
            hu_line = hu_data

            exit = torch.round(exit)
            entry = torch.round(entry)

            fine_tune_optimizer.zero_grad()
            preds, logits = model(so, td, entry, exit, hu_line, td_idx)
            ...
            soft_target = make_circular_soft_label(soft_label.float(), num_bins=num_bins, smoothing_radius=1, sigma=0.3)
            loss = 0.5*phase_loss_fn(log_probs, soft_target) + 0.5*circ_loss
        ...
        mae_val = total_mae/len(ref_loader)
        print(f'{fine_epoch+1}/50 || Fine Loss: {total_loss/len(ref_loader):.6f} '
              f'|| MAE: {mae_val:.6f} || Skull {sk_idx}, Area {aidx}')

        model.eval()
        val_dataset = MultiSkullDataset(
            skull=skull, nbr_skull_list=[sk_idx], so_vxl=so_vxl, td_vxl=td_vxl,
            ph_list=ph_list, target_class=target_class, td_idx_list=[td_idx], so_idx_list=val_points
        )
        val_loader = DataLoader(val_dataset, batch_size=8, shuffle=False)

        total_mae_val = 0.0
        total_circ_loss = total_huber_loss = 0.0
        start_time = time.time()
        with torch.no_grad():
            for skull_idx, so, td, gt_phase, soft_label, vol in val_loader:
                so, td, gt_phase = so.to(device), td.to(device), gt_phase.to(device)
                vol = vol.to(device)
                ...
                preds, log_probs = model(so, td, entry, exit, hu_line, td_idx)
                mae = torch.mean(torch.abs(torch.atan2(torch.sin(preds - gt_phase),
                                                        torch.cos(preds - gt_phase))))
                diff = torch.atan2(torch.sin(preds - gt_phase), torch.cos(preds - gt_phase))
                circ_loss = 1 - torch.cos(diff)
                huber_loss = F.huber_loss(diff, torch.zeros_like(diff), delta=0.2, reduction='mean')
                
                total_mae_val += mae.item()
                total_circ_loss += circ_loss.mean().item()
                total_huber_loss += huber_loss.item()
        end_time = time.time()
    end_tuning = time.time()

    inference_time = (end_time - start_time) / len(val_loader)
